...
# warning-ignore:return_value_discarded
	if "users/" in path:
		http.request(url, _get_request_headers(token), HTTPClient.METHOD_POST, body)
...
func get_document(path: String, http: HTTPRequest, token: String, player_container)-> void:
	var url := DATABASE_URL + path
# warning-ignore:return_value_discarded
	http.request(url, _get_request_headers(token), HTTPClient.METHOD_GET)
...
func update_document(path: String, http: HTTPRequest, token: String, player_container) -> void:
	...
	if 'users/' in path:
		...
		var url := DATABASE_URL + path
		# warning-ignore:return_value_discarded
		http.request(url, _get_request_headers(token), HTTPClient.METHOD_PATCH, body)
		await http.request_completed
	else:
		# update /character
...
func delete_document(path: String, http: HTTPRequest, token: String) -> void:
	var url := DATABASE_URL + path
# warning-ignore:return_value_discarded
	http.request(url, _get_request_headers(token), HTTPClient.METHOD_DELETE)
	await http.request_completed
